...
function idValidCheck(_form){
	...
	if(!regex.test(_form.id.value)){
		...
		_form.idcheck.className = 'btn btn-primary btn-sm disabled' // 중복 체크 버튼 비활성화
		iv.innerText = 'ID is invalid.'
		_form.dupck.value = 0 // 중복 체크 미실시로 변경
	}else{
		_form.id.className = 'form-control is-valid';
		v.innerText = 'ID is valid'
		_form.idcheck.className = 'btn btn-primary btn-sm' // 중복 체크 버튼 활성화
		_form.dupck.value = 0
	}
}
// 회원가입 입력 정보 검사
